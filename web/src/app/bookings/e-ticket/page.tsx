...
function ETicketContent() {
  ...
  const passenger         = sp.get("passenger") ?? "";
  const pTitle            = sp.get("pTitle") ?? "";
  const seat              = sp.get("seat") ?? "-";
  const flightNumber      = sp.get("flightNumber") ?? "";
  const airline           = sp.get("airline") ?? "";
  const departureIso      = sp.get("departureIso") ?? "";
  const arrivalIso        = sp.get("arrivalIso") ?? "";
  const originAirportName = sp.get("originAirportName") ?? "";
  const destAirportName   = sp.get("destAirportName") ?? "";
  const originCity        = sp.get("originCity") ?? sp.get("route")?.split("→")[0]?.trim() ?? "";
  const destCity          = sp.get("destCity") ?? sp.get("route")?.split("→")[1]?.trim() ?? "";
  const pDocType          = sp.get("pDocType") ?? "";
  const pDocNumber        = sp.get("pDocNumber") ?? "";
  const totalPrice        = sp.get("totalPrice") ?? "";
  const date              = sp.get("date") ?? fmtDateEn(departureIso);
    ...
    <>
      <style>{`
        @media print {
          ...
          .ticket-doc { box-shadow: none !important; border: none !important; max-width: 100% !important; margin: 0 !important; }
        ...
        <div className="ticket-doc mx-auto max-w-2xl overflow-hidden rounded-xl border border-gray-200 bg-white shadow-md">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                <tr>
                  ...
                  <td className="pt-3 font-semibold text-gray-900 align-top">
                    {pTitle ? `${pTitle}. ` : ""}{passenger || "—"}
                  </td>
          ...
          <div className="no-print border-t border-gray-100 bg-gray-50 px-8 py-3 text-center">
            <p className="text-[11px] italic text-gray-400">
              Contoh Electronic Ticket (E-Ticket) Penerbangan · SkyIntern E-Ticketing System
            </p>
          </div>
        </div>
      </main>
