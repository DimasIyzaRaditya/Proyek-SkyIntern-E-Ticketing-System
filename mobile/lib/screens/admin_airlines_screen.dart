...
class _AdminAirlinesScreenState extends State<AdminAirlinesScreen> {
  final List<Map<String, dynamic>> _airlines = [
    {'id': 1, 'name': 'Garuda Indonesia', 'iataCode': 'GA', 'country': 'Indonesia'},
    {'id': 2, 'name': 'Lion Air', 'iataCode': 'JT', 'country': 'Indonesia'},
    {'id': 3, 'name': 'Batik Air', 'iataCode': 'ID', 'country': 'Indonesia'},
  ];
  ...
  void _showAddEditDialog({Map<String, dynamic>? airline}) {
    ...
    final codeCtrl = TextEditingController(text: airline?['iataCode'] ?? '');
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              ...
              if (isEdit) {
                setState(() {
                  final idx = _airlines.indexWhere((a) => a['id'] == airline!['id']);
                  if (idx != -1) {
                    _airlines[idx] = {...airline!, 'name': nameCtrl.text, 'iataCode': codeCtrl.text, 'country': countryCtrl.text};
                  }
                });
              } else {
                setState(() => _airlines.add({'id': _airlines.length + 1, 'name': nameCtrl.text, 'iataCode': codeCtrl.text, 'country': countryCtrl.text}));
              }
              Navigator.pop(ctx);
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              setState(() => _airlines.removeWhere((a) => a['id'] == airline['id']));
              Navigator.pop(ctx);
